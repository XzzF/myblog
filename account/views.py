...
def user_login(request):
    if request.method == 'POST':
        ...
        if login_form.is_valid():
            ...
            if user:
                login(request, user)
                return HttpResponse("Authenticate successful, welcome to login.")
            else:
                return HttpResponse("Sorry, authenticate failed.")
...
def user_register(request):
    if request.method == 'POST':
        ...
        return render(request, "account/register.html", {"form": user_form, "proflie": userprofile_form})
...
def myself(request):
    ...
    userprofile = UserProfile.objects.get(user=user) if hasattr(user, 'userprofile') else UserProfile.object.create(user=user)
